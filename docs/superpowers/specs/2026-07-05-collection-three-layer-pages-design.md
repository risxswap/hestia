...
- `衣服` 页已有独立详情页和编辑页，且新增字段较少，可以作为轻量新增弹层例外保留在列表页。
...
- `我的` 页改成档案概览和入口页。
- `衣服` 新增字段较少，允许作为轻量例外继续使用列表页新增弹层；详情页仍必须只读。
...
- 不为了兼容旧页面结构保留同页大表单；衣服新增弹层是基于字段少和操作轻的产品例外，不是兼容旧实现。
...
- 展示摘要、计数、状态、筛选和入口。
- 展示轻量卡片，卡片点击进入详情页。
- 新增按钮默认跳转到独立新增页；衣服新增作为轻量例外，保留列表页新增弹层。
- 不展示大表单，不承载上传和保存。
...
- 新增弹层不得扩展为完整衣服编辑表单；如果新增字段增长，应重新迁移到独立新增页。
...
6. 衣服页面收口：保留列表页轻量新增弹层，确保详情页只读、编辑页只处理已有衣服。
