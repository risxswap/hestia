...
CREATE TABLE IF NOT EXISTS `chat_msgs` (
  ...
  KEY `idx_chat_msgs_source_msg` (`source_msg_id`),
...
CREATE TABLE IF NOT EXISTS `agent_run_steps` (
  ...
  `created_at` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_agent_run_steps_public_id` (`public_id`),
  UNIQUE KEY `uk_agent_run_steps_assistant_step` (`assistant_msg_id`, `step_no`),
  KEY `idx_agent_run_steps_source_msg` (`source_msg_id`),
  KEY `idx_agent_run_steps_assistant_msg` (`assistant_msg_id`),
  KEY `idx_agent_run_steps_user_created` (`user_id`, `created_at`),
  KEY `idx_agent_run_steps_related` (`related_type`, `related_id`),
  KEY `idx_agent_run_steps_tool_call` (`assistant_msg_id`, `tool_call_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

CREATE TABLE IF NOT EXISTS `advice_drafts` (
  `id` bigint unsigned NOT NULL AUTO_INCREMENT,
  `public_id` varchar(32) NOT NULL,
  `user_id` bigint unsigned NOT NULL,
  `source_msg_id` bigint unsigned DEFAULT NULL,
  `status` varchar(32) NOT NULL DEFAULT 'draft',
  `scene_key` varchar(64) DEFAULT NULL,
  ...
  `confirmed_advice_id` bigint unsigned DEFAULT NULL,
  `created_at` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  `updated_at` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
  `deleted_at` datetime(3) DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_advice_drafts_public_id` (`public_id`),
  KEY `idx_advice_drafts_user_status_updated` (`user_id`, `status`, `updated_at`),
  KEY `idx_advice_drafts_source_msg` (`source_msg_id`),
  KEY `idx_advice_drafts_confirmed_advice` (`confirmed_advice_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

CREATE TABLE IF NOT EXISTS `advice_draft_sections` (
  `id` bigint unsigned NOT NULL AUTO_INCREMENT,
  `public_id` varchar(32) NOT NULL,
  `draft_id` bigint unsigned NOT NULL,
  `user_id` bigint unsigned NOT NULL,
  `section_type` varchar(32) NOT NULL,
  `current_section_version_id` bigint unsigned DEFAULT NULL,
  `current_section_version_no` int unsigned NOT NULL DEFAULT 1,
  `content_schema_version` varchar(32) NOT NULL DEFAULT 'v1',
  `content_json` json NOT NULL,
  `created_at` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  `updated_at` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
  `deleted_at` datetime(3) DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_advice_draft_sections_public_id` (`public_id`),
  UNIQUE KEY `uk_advice_draft_sections_draft_type` (`draft_id`, `section_type`),
  KEY `idx_advice_draft_sections_user` (`user_id`),
  KEY `idx_advice_draft_sections_current_version` (`current_section_version_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

CREATE TABLE IF NOT EXISTS `advice_draft_section_versions` (
  `id` bigint unsigned NOT NULL AUTO_INCREMENT,
  `public_id` varchar(32) NOT NULL,
  `draft_id` bigint unsigned NOT NULL,
  ...
  `user_id` bigint unsigned NOT NULL,
  `section_type` varchar(32) NOT NULL,
  `section_version_no` int unsigned NOT NULL,
  `draft_revision_no` int unsigned NOT NULL,
  `source_msg_id` bigint unsigned DEFAULT NULL,
  `user_intent` text,
  ...
  `created_at` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_advice_draft_section_versions_public_id` (`public_id`),
  UNIQUE KEY `uk_advice_draft_section_versions_section_version` (`draft_id`, `section_type`, `section_version_no`),
  UNIQUE KEY `uk_advice_draft_section_versions_revision_type` (`draft_id`, `draft_revision_no`, `section_type`),
  KEY `idx_advice_draft_section_versions_section` (`section_id`),
  KEY `idx_advice_draft_section_versions_user_created` (`user_id`, `created_at`),
  KEY `idx_advice_draft_section_versions_source_msg` (`source_msg_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
CREATE TABLE IF NOT EXISTS `advices` (
  ...
  KEY `idx_advices_user_target_scene` (`user_id`, `target_date`, `scene_key`, `status`),
  KEY `idx_advices_source_msg` (`source_msg_id`),
  KEY `idx_advices_source_draft` (`source_draft_id`, `source_draft_revision_no`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
CREATE TABLE IF NOT EXISTS `advice_sections` (
  ...
  `updated_at` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_advice_sections_public_id` (`public_id`),
