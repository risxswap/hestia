...
var ErrImageRecognizerUnavailable = errors.New("wardrobe image recognizer unavailable")
...
func (r *LLMImageRecognizer) RecognizeWardrobeItemImage(ctx context.Context, userID int64, input RecognizeImageInput) (RecognizedItemFields, error) {
	if r == nil || r.generator == nil {
		return RecognizedItemFields{}, ErrImageRecognizerUnavailable
	}
	prompt := wardrobeImageRecognizePrompt(userID, input.AssetPublicID)
	request := llm.Request{
	...
	if err := json.Unmarshal([]byte(extractJSONObject(response.Text)), &result); err != nil {
		return RecognizedItemFields{}, fmt.Errorf("parse wardrobe image recognition result: %w", err)
	}
	return result, nil
}

func wardrobeImageRecognizePrompt(userID int64, assetPublicID string) string {
