...
func (h *Handler) ListItems(c *gin.Context) {
	...
	if err != nil {
		h.writeError(c, err, "list hair items failed", user.UserID, "")
		return
	}
...
func (h *Handler) GetItem(c *gin.Context) {
	...
	if err != nil {
		h.writeError(c, err, "get hair item failed", user.UserID, publicID)
		return
	}
	response.OK(c, item)
}
...
func (h *Handler) CreateItem(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&input); err != nil {
		response.Error(c, http.StatusBadRequest, "hair.invalid_request", "请求参数不正确")
		return
	}
	item, err := h.service.CreateItem(c.Request.Context(), user.UserID, input)
	if err != nil {
		h.writeError(c, err, "create hair item failed", user.UserID, "")
		return
	}
	response.OK(c, item)
}
...
func (h *Handler) UpdateItem(c *gin.Context) {
	...
	if err != nil {
		h.writeError(c, err, "update hair item failed", user.UserID, publicID)
		return
	}
	response.OK(c, item)
}
...
func (h *Handler) DeleteItem(c *gin.Context) {
	...
	if err := h.service.SoftDeleteItem(c.Request.Context(), user.UserID, publicID); err != nil {
		h.writeError(c, err, "delete hair item failed", user.UserID, publicID)
		return
	}
...
func (h *Handler) writeError(c *gin.Context, err error, logMessage string, userID int64, publicID string) {
	...
	case errors.Is(err, ErrInvalidRecommendationStatus):
		response.Error(c, http.StatusBadRequest, "hair.invalid_recommendation_status", "不支持的推荐状态")
	case errors.Is(err, ErrInvalidItemName):
		response.Error(c, http.StatusBadRequest, "hair.invalid_item", "发型名称不能为空")
	case errors.Is(err, ErrInvalidPrimaryAsset):
		response.Error(c, http.StatusBadRequest, "hair.invalid_primary_asset", "主图资产不可用于发型")
	case errors.Is(err, ErrItemNotFound):
		response.Error(c, http.StatusNotFound, "hair.item_not_found", "发型不存在")
	default:
		h.logger.Error(logMessage, "error", err, "user_id", userID, "item_public_id", publicID)
		response.Error(c, http.StatusInternalServerError, "hair.request_failed", "发型请求失败")
	}
}
