...
var (
	ErrInvalidRecommendationStatus = errors.New("invalid makeup recommendation status")
	ErrInvalidItemName             = errors.New("invalid makeup name")
	ErrInvalidPrimaryAsset         = errors.New("invalid makeup primary asset")
	ErrItemNotFound                = errors.New("makeup item not found")
	ErrRepositoryUnsupported       = errors.New("makeup repository unsupported")
)
...
func (s *Service) CreateItem(ctx context.Context, userID int64, input CreateInput) (Item, error) {
	...
	now := time.Now().UTC()
	item := Item{
		PublicID:             id.NewPublicID("mku"),
		UserID:               userID,
		Name:                 name,
		MakeupType:           strings.TrimSpace(input.MakeupType),
		Focus:                strings.TrimSpace(input.Focus),
		ColorPalette:         strings.TrimSpace(input.ColorPalette),
		Finish:               strings.TrimSpace(input.Finish),
...
func (s *Service) UpdateItem(ctx context.Context, userID int64, publicID string, input UpdateInput) (Item, error) {
	...
	trimStringPtr(input.MakeupType)
	trimStringPtr(input.Focus)
	trimStringPtr(input.ColorPalette)
	trimStringPtr(input.Finish)
