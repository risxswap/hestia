...
function optionsState(rawOptions) {
  ...
  return {
    ...
    colorOptions: clothesOptions.colors,
    materialOptions: clothesOptions.materials,
    seasonOptions: clothesOptions.seasons,
    silhouetteOptions: clothesOptions.silhouettes
  };
}
...
const clothesDetailPageConfig = {
  data: {
    ...
    item: null,
    backLabel: "返回私藏",
    backUrl: "/pages/collection/collection",
    backMode: "switchTab",
    clothesDirty: false,
    editorVisible: false,
    draft: cloneDraft(),
    ...
    colorOptions: normalizeClothesOptions().colors,
    materialOptions: normalizeClothesOptions().materials,
    seasonOptions: normalizeClothesOptions().seasons,
    silhouetteOptions: normalizeClothesOptions().silhouettes,
  ...
  onLoad(options) {
    ...
    const fromClothes = source === "clothes";
    this.setData({
      backLabel: fromClothes ? "返回衣服" : "返回私藏",
      backUrl: fromClothes ? "/pages/clothes/list" : "/pages/collection/collection",
      backMode: fromClothes ? "navigateBack" : "switchTab"
    });
  ...
  async loadClothesItem(publicID) {
    ...
    try {
      ...
      const itemRequest = typeof api.getClothesItem === "function" ? api.getClothesItem(publicID) : api.getClothesItems();
      const results = await Promise.all([itemRequest, optionsRequest]);
      const optionState = optionsState(results[1]);
      const item = typeof api.getClothesItem === "function"
        ? decorateClothesItem(results[0])
        : normalizeClothesItems(results[0]).find((entry) => entry.public_id === publicID);
      if (!item) {
  ...
  returnToSource() {
    if (this.data.backMode === "navigateBack" && typeof wx !== "undefined" && wx.navigateBack) {
      wx.navigateBack({
        delta: 1,
        fail: () => {
          if (typeof wx !== "undefined" && wx.redirectTo) {
            wx.redirectTo({
              url: this.data.backUrl || "/pages/clothes/list"
            });
          }
        }
      });
    ...
    if (this.data.backMode === "navigateBack" && typeof wx !== "undefined" && wx.redirectTo) {
      wx.redirectTo({
        url: this.data.backUrl || "/pages/clothes/list"
      });
      return;
    ...
    if (typeof wx !== "undefined" && wx.switchTab) {
      wx.switchTab({
        url: this.data.backUrl || "/pages/collection/collection"
      });
    }
  },
  ...
  handleOpenEdit() {
    ...
    if (typeof wx !== "undefined" && wx.navigateTo) {
      wx.navigateTo({
        url: `/pages/clothes/edit?public_id=${this.data.item.public_id}`
      });
    }
  },
  ...
  async handleRecognizeImageOverwrite() {
    ...
    try {
      const recognized = await api.recognizeClothesItemImage(assetPublicID, {
  ...
  async handleSaveItem() {
    ...
    try {
      const saved = await api.updateClothesItem(publicID, payload);
      const item = decorateClothesItem(saved);
      this.setData(Object.assign({
        saving: false,
        editorVisible: false,
        editingPublicID: "",
...
if (typeof Page === "function") {
  Page(clothesDetailPageConfig);
}

if (typeof module !== "undefined") {
  module.exports = {
    clothesDetailPageConfig
  };
}
