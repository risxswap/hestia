...
const clothesUtils = require("../../utils/clothes");
...
function nextClothesState(items, category) {
  return {
    items,
...
const clothesPageConfig = {
  data: {
    ...
    clothesOptions: normalizeClothesOptions(),
    editorCategoryOptions: normalizeClothesOptions().categories,
    materialOptions: withEmptyOption(normalizeClothesOptions().materials),
    seasonOptions: withEmptyOption(normalizeClothesOptions().seasons),
    silhouetteOptions: withEmptyOption(normalizeClothesOptions().silhouettes),
    activeCategory: "all",
    clothesDirty: false,
    editorVisible: false,
  ...
  async loadClothes() {
    ...
    try {
      const optionsRequest = typeof api.getClothesOptions === "function" ? api.getClothesOptions() : Promise.resolve(null);
      const reportRequest = typeof api.getLatestReport === "function" ? api.getLatestReport() : Promise.resolve(null);
      const itemRequest = typeof api.getClothesItems === "function" ? api.getClothesItems() : Promise.resolve({ items: [] });
      const results = await Promise.all([itemRequest, reportRequest, optionsRequest]);
      const items = normalizeClothesItems(results[0]);
      ...
      this.setData(Object.assign({}, optionsState, draftState(this.data.draft, optionsState), {
        loading: false,
        gaps: normalizeClothesGaps(results[1])
      }, nextClothesState(items, this.data.activeCategory)));
    } catch (error) {
      this.setData({
        loading: false,
        items: [],
  ...
  optionsState(rawOptions) {
    const clothesOptions = normalizeClothesOptions(rawOptions);
    return {
      clothesOptions,
      editorCategoryOptions: clothesOptions.categories,
      materialOptions: withEmptyOption(clothesOptions.materials),
      seasonOptions: withEmptyOption(clothesOptions.seasons),
      silhouetteOptions: withEmptyOption(clothesOptions.silhouettes)
    };
  },

  async loadClothesOptions() {
    try {
      const options = typeof api.getClothesOptions === "function" ? await api.getClothesOptions() : null;
  ...
  async loadClothesGaps() {
    ...
    try {
      ...
      this.setData({
        loading: false,
        gaps: normalizeClothesGaps(report)
      });
    } catch (error) {
      this.setData({
        loading: false,
  ...
  handleOpenDetail(event) {
    ...
    if (typeof wx !== "undefined" && wx.navigateTo) {
      wx.navigateTo({
        url: `/pages/clothes/detail?public_id=${publicID}&source=clothes`
      });
    }
  },
  ...
  handleEditItem(event) {
    ...
    if (typeof wx !== "undefined" && wx.navigateTo) {
      wx.navigateTo({
        url: `/pages/clothes/edit?public_id=${publicID}`
      });
    }
  },
  ...
  async recognizeUploadedClothesImage(uploaded, uploadRunID) {
    return this.applyUploadedClothesRecognition(uploaded, uploadRunID);
  },

  async applyUploadedClothesRecognition(uploaded, uploadRunID) {
    const assetPublicID = uploaded && uploaded.asset_public_id ? uploaded.asset_public_id : "";
    ...
    try {
      const recognized = confirmedFields || (typeof api.recognizeClothesItemImage === "function"
        ? await api.recognizeClothesItemImage(assetPublicID)
        : null);
      ...
      this.setData({
        draft: decorateDraftForOptions(mergeRecognizedFieldsIntoDraft(this.data.draft, recognized, this.data.clothesOptions), this.data),
        imageRecognizing: false,
        imageRecognizeError: ""
      });
      return recognized;
  ...
  async createItemFromUploadedImage(uploaded, uploadRunID) {
    ...
    try {
      const recognized = uploaded && uploaded.recognized_fields
        ...
        : typeof api.recognizeClothesItemImage === "function"
          ? await api.recognizeClothesItemImage(assetPublicID)
          : null;
      if (this._imageUploadRunID !== recognizeRunID) {
        return recognized;
      ...
      })), recognized || {}, this.data.clothesOptions);
      ...
      const saved = await api.createClothesItem(payload);
      if (this._imageUploadRunID !== recognizeRunID) {
        return saved;
      }
      const normalizedSaved = decorateClothesItem(saved);
      ...
      this.setData(Object.assign({
        ...
        imageRecognizeError: "",
        errorMessage: ""
      }, nextClothesState(nextItems, this.data.activeCategory)));
  ...
  async handleSaveItem() {
    ...
    try {
      const saved = this.data.editingPublicID
        ? await api.updateClothesItem(this.data.editingPublicID, payload)
        : await api.createClothesItem(payload);
      const normalizedSaved = decorateClothesItem(saved);
  ...
  async handleDeleteItem(event) {
    ...
    try {
      await api.deleteClothesItem(publicID);
